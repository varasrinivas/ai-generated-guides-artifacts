...
Run this under a read-only sandbox. Codex skills carry no per-skill
tool list, so the sandbox is what makes "audit, not fix" true rather
than merely requested: `codex --profile boot4-assess`, or the
behavior-auditor subagent, which pins `sandbox_mode = "read-only"`.
