...
- Vendoring produces branch `chore/boot4-kit-<rev>` with `.agents/skills/`,
  `.codex/agents/`, and `.codex/hooks.json` (Codex reads the hooks file
  directly — no merge step); the PR review is the adoption gate.
- `.codex/config.toml` (sandbox and approval policy) is per-repo and is NOT
  copied — start from `examples/config.toml`.
...
1. Fresh context (a new session with no stake in the result): review the whole branch diff hunting
...
```
.agents/ .codex/…                 kit (skills, agents, compile hook)
