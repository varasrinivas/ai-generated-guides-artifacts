...
The kit ships by vendoring — the sync PR is the adoption gate:
...
- Vendoring produces branch `chore/boot4-kit-<rev>` with `.github/skills/`,
  `.github/agents/`, and `.github/hooks/boot4.json` (Copilot reads repo hooks
  from `.github/hooks/*.json` directly — no merge step); the PR review is the
  adoption gate. Note the hook applies to every Copilot agent run in the repo,
  CLI and cloud alike — that is what the PR review accepts.
- Tool approvals are not repo files: they are session flags
  (`--allow-tool 'shell(./mvnw:*)'`) or saved per-location approvals in
  `~/.copilot/permissions-config.json`. The phase lines below carry the flags.
- The phase rule files land at
  `.github/skills/boot4-migration/references/phases/phase-0..7.md` — each phase
  below starts by reading its file. (`.github` is a dot-directory; use
  `git ls-files .github` if your tree view hides them.)
...
## Phase 0 — assessment (plan mode: `Shift+Tab` or `/plan`, no edits)
...
1. Dispatch the `test-migrator` custom agent per module. It may not touch
   src/main and may never weaken an assertion; behavioral failures come back
   as findings.
...
2. Dispatch the read-only `behavior-auditor` custom agent (strongest model
   tier). Give it the assessment's Tier-3 list and let it probe the running
   app. Disposition all findings; commit fixes separately with the finding
   named.
...
1. Fresh context (`/clear`, or a new session with no stake in the result):
   review the whole branch diff hunting weakened assertions, smuggled behavior
   changes, unrelated refactors, leftover shims, and doc claims that aren't
   true in the code. Address docs-only findings as their own commit.
...
```
.github/skills|agents|hooks…      kit (skills, agents, compile hook)
