...
**How to run:** Agentic loop. Session-approved tools for mechanical categories (imports,
package moves, starter renames); clear the approvals (`/permissions reset`) and confirm
each edit for Security DSL rewrites — Security 7 defaults can silently change endpoint
behavior.
